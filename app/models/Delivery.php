...
{
    $this->db->query('SELECT * FROM onlineorder INNER JOIN onlineprescription ON onlineorder.onlineoid = onlineprescription.onlineorderid INNER JOIN bill ON onlineprescription.onlinepresid = bill.presid ORDER BY bill.billid DESC');
...
public function getdelcustdata($delpresid) {
    $this->db->query('SELECT * FROM onlineorder INNER JOIN onlineprescription ON onlineorder.onlineoid = onlineprescription.onlineorderid INNER JOIN bill ON onlineprescription.onlinepresid = bill.presid WHERE bill.presid = :delpresid');
...
public function getdelpresdata($delpresid) {
    $this->db->query('SELECT * FROM onlinepresmed INNER JOIN medicine ON medicine.medid= onlinepresmed.medid  WHERE onlinepresid = :presid');
...
public function confirmdel($data){
    $this->db->query('UPDATE delivery SET delstatus = :delstat, delivereddate = :deldate, deltime = :deltime WHERE presid = :presid');
